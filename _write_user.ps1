      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="toolbar-btns">
            ...
            <el-button type="danger" :disabled="selectedIds.length === 0" v-hasPermi="['system:user:remove']" @click="handleBatchDelete"><el-icon><Delete /></el-icon>{{ `$`+`{t('common.batchDelete')}` }}</el-button>
      ...
      <el-table v-loading="loading" :data="userList" border stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column :label="`$`+`{t('system.user.status')}`" prop="status" width="100" align="center">
          <template #default="{ row }">
            <el-switch v-model="row.status" :active-value="0" :inactive-value="1" @change="handleStatusChange(row)" v-hasPermi="['system:user:edit']" />
          </template>
        </el-table-column>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item :label="`$`+`{t('system.user.roles')}`" prop="roleIds">
              <el-select v-model="form.roleIds" multiple :placeholder="`$`+`{t('common.selectPlaceholder')}`" style="width: 100%">
...
async function loadRoleOptions() {
  try {
    const res: any = await roleApi.list({ pageNum: 1, pageSize: 999 })
    roleOptions.value = res.data?.records || res.data?.list || []
  } catch (_e) {
    roleOptions.value = []
  }
}
...
"@
[System.IO.File]::WriteAllText("d:\zhehang-erp\zhehang-erp-ui\src\views\system\user.vue", $content, [System.Text.Encoding]::UTF8)
