...
public class CompanyInfoService {
    ...
    public Map<String, Object> page(String keyword, String region, String risk, int pageNum, int pageSize) {
        ...
        // 用 long 计算偏移并夹紧,避免超大 pageNum 时 int 乘法溢出为负数导致 subList 越界。
        long offset = (long) (pn - 1) * ps;
        int from = (int) Math.min(Math.max(offset, 0), filtered.size());
        int to = (int) Math.min((long) from + ps, filtered.size());
