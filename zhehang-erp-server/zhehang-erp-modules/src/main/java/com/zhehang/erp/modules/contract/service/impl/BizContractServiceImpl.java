...
public class BizContractServiceImpl extends ServiceImpl<BizContractMapper, BizContract> implements IBizContractService {
    ...
    public Long generateFromOrder(Long orderId, Long templateId, String title) {
        ...
        if (!dataScopeHelper.canAccess(order.getSalesmanId(), order.getDeptId())) {
            throw new AccessDeniedException("无权为数据范围外的订单生成合同");
        }
    ...
    public void sendSign(Long id, String signMode) {
        BizContract contract = requireManagerAccess(id, "发起签署");
    ...
    public void confirmSign(Long id, String signerTheirs) {
        BizContract contract = requireManagerAccess(id, "确认签署");
        if (contract.getStatus() != null && contract.getStatus() >= 4) {
    ...
    public Long renew(Long id, String newTitle) {
        BizContract origin = requireManagerAccess(id, "续签");
    ...
    public void terminate(Long id, String reason) {
        BizContract contract = requireManagerAccess(id, "终止");
    ...
    /** 合同状态变更必须同时满足“管理角色 + 该合同在数据范围内”，服务层二次防线防止绕过 Controller。 */
    private BizContract requireManagerAccess(Long id, String action) {
        if (!dataScopeHelper.isManagerOrAdmin()) {
            throw new AccessDeniedException("无权" + action + "合同");
        }
        ...
        if (!dataScopeHelper.canAccess(contract.getSalesmanId(), contract.getDeptId())) {
            throw new AccessDeniedException("无权" + action + "数据范围外的合同");
        }
        return contract;
    }
    ...
    public Long saveTemplate(BizContractTemplate template) {
        template.setContent(SafeHtmlSanitizer.sanitize(template.getContent()));
