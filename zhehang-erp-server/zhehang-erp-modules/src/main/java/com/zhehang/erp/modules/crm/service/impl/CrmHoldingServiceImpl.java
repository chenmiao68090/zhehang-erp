...
public class CrmHoldingServiceImpl extends ServiceImpl<CrmHoldingConfigMapper, CrmHoldingConfig>
    ...
    /** 无任何配置时的默认持有上限 */
    private static final int DEFAULT_MAX_HOLDING = 1000;
    ...
    public List<CrmHoldingConfig> listEnabled() {
        ...
        wrapper.eq(CrmHoldingConfig::getStatus, 1)
    ...
    private int resolveMaxHolding() {
        try {
            Integer configured = ruleService.current().getPrivateHoldingLimit();
            return configured == null ? DEFAULT_MAX_HOLDING : configured;
