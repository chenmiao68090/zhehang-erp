...
public class BizReceiptServiceImpl extends ServiceImpl<BizReceiptMapper, BizReceipt> implements IBizReceiptService {
    ...
    public void confirm(BizReceipt receipt, Long operatorId) {
        // 幂等守卫:已确认(status=2)的收款不可重复确认,否则带planId的收款会重复累加计划已收额(模仿 BizContractServiceImpl.confirmSign)
        if (receipt.getId() != null) {
            BizReceipt db = receiptMapper.selectById(receipt.getId());
            if (db != null && Integer.valueOf(2).equals(db.getStatus())) {
                throw new BusinessException("该收款已确认,请勿重复确认");
            }
        }
    ...
    public void refund(Long receiptId, BigDecimal refundAmount, String reason, Long operatorId) {
        ...
        if (!Integer.valueOf(2).equals(receipt.getStatus())) {
            throw new BusinessException("只有已确认收款可以退款");
        }
        ...
        receiptMapper.updateById(receipt);
        // 退款需对称冲减关联收款计划的已收额,否则 plan.paidAmount/状态与实际不符,导致逾期判断与财务报表错数
        if (receipt.getPlanId() != null) {
            BizReceiptPlan plan = planMapper.selectById(receipt.getPlanId());
            if (plan != null) {
                BigDecimal paid = plan.getPaidAmount() == null ? BigDecimal.ZERO : plan.getPaidAmount();
                BigDecimal newPaid = paid.subtract(refundAmount);
                if (newPaid.compareTo(BigDecimal.ZERO) < 0) newPaid = BigDecimal.ZERO;
                plan.setPaidAmount(newPaid);
                // 状态镜像 payPlan:已收齐3 / 部分2 / 未收1
                if (plan.getPlanAmount() != null && newPaid.compareTo(plan.getPlanAmount()) >= 0) {
                    plan.setStatus(3);
                } else if (newPaid.compareTo(BigDecimal.ZERO) > 0) {
                    plan.setStatus(2);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void reject(Long receiptId, String reason, Long operatorId) {
        BizReceipt receipt = receiptMapper.selectById(receiptId);
        if (receipt == null) {
            throw new BusinessException("收款记录不存在");
        }
        receipt.setStatus(4); // 已驳回
        receipt.setConfirmerId(operatorId);
        receipt.setConfirmTime(LocalDateTime.now());
        receipt.setRemark((receipt.getRemark() == null ? "" : receipt.getRemark())
                + " | 驳回: " + (reason == null ? "" : reason));
        receiptMapper.updateById(receipt);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public BizReceipt postReceipt(Long receiptId, String postingAccount, Long operatorId) {
        BizReceipt receipt = receiptMapper.selectById(receiptId);
        if (receipt == null) {
            throw new BusinessException("收款记录不存在");
        }
        if (!Integer.valueOf(2).equals(receipt.getStatus())) {
            throw new BusinessException("只有已确认收款可以入账");
        }
        if (StringUtils.hasText(receipt.getPostingAccount())) {
            throw new BusinessException("该收款已入账，不可重复入账");
        }
        if (!StringUtils.hasText(postingAccount)) {
            throw new BusinessException("请选择入账科目");
        }
        receipt.setPostingAccount(postingAccount.trim());
        receipt.setPostingOperatorId(operatorId);
        receipt.setPostingTime(LocalDateTime.now());
        receiptMapper.updateById(receipt);
        return receipt;
    }
    ...
    public IPage<BizInvoice> getInvoices(int pageNum, int pageSize, Long customerId, Integer status) {
        ...
        // 数据范围:财务/管理员(data_scope=1)看全部发票;其余只看本人开具的(create_by=当前用户)
        dataScopeHelper.applyCreatorScope(wrapper, BizInvoice::getCreateBy);
    ...
    public void updateInvoiceStatus(Long id, Integer status, String trackingNo) {
        ...
        if (!dataScopeHelper.canAccess(invoice.getCreateBy(), null)) {
            throw new BusinessException("无权操作他人发票");
        }
        if (status != null) {
    ...
    public Map<String, Object> monthlySummary(String month) {
        ...
        LambdaQueryWrapper<BizReceipt> wrapper = new LambdaQueryWrapper<>();
        // 数据范围:与 selectPage 同口径,销售看本人、主管看本部门、财务/管理员/老板看全部
        dataScopeHelper.applyFinancial(wrapper, BizReceipt::getSalesmanId, BizReceipt::getDeptId);
