...
public interface IBizTaskService extends IService<BizTask> {
    ...
    /** 交接列表 */
    IPage<BizTaskHandover> handoverList(int pageNum, int pageSize, Long customerId, Integer status);

    /** 创建交接 */
    Long createHandover(BizTaskHandover handover, List<BizTaskHandoverItem> items);

    /** 更新交接子项 */
    void updateHandoverItem(Long handoverId, BizTaskHandoverItem item);
