//#region src/utils/lead-import.ts
var LEAD_IMPORT_FIELDS = [
	{
		key: "company",
		label: "企业名称",
		level: "required",
...
function normalizeHeader(value) {
	return String(value !== null && value !== void 0 ? value : "").trim().toLowerCase().replace(/[\s_\-—（）()【】\[\]·:：/\\]/g, "");
}
function autoMapLeadImportHeaders(headers) {
	const normalized = new Map(headers.map((header) => [normalizeHeader(header), header]));
	const mapping = {};
	for (const field of LEAD_IMPORT_FIELDS) {
		const exact = [field.label, ...field.aliases].map(normalizeHeader).map((name) => normalized.get(name)).find(Boolean);
		if (exact) mapping[field.key] = exact;
	}
	return mapping;
}
function parseLeadImportCsv(text) {
	const matrix = [];
	let row = [];
	let cell = "";
	let quoted = false;
...
function normalizeLeadImportMatrix(matrix) {
	const normalized = matrix.filter(Array.isArray).map((row) => row.map((cell) => {
		if (cell instanceof Date && !Number.isNaN(cell.getTime())) return `${cell.getFullYear()}-${String(cell.getMonth() + 1).padStart(2, "0")}-${String(cell.getDate()).padStart(2, "0")}`;
...
function normalizeDate(value) {
	...
	if (/^\d{4,5}(?:\.\d+)?$/.test(normalized)) {
		...
		if (serial > 0 && serial < 6e4) {
			const date = new Date(Date.UTC(1899, 11, 30) + Math.floor(serial) * 864e5);
...
function sourceCell(source, mapping, key) {
	var _source$header;
	const header = mapping[key];
	return header ? String((_source$header = source[header]) !== null && _source$header !== void 0 ? _source$header : "").trim() : "";
}
function buildLeadImportRows(matrix, mapping) {
...
function escapeCsvCell(value) {
	...
	return `"${(/^[=+\-@]/.test(raw.trimStart()) ? `'${raw}` : raw).replace(/"/g, "\"\"")}"`;
}
//#endregion
export { normalizeLeadImportMatrix as a, escapeCsvCell as i, autoMapLeadImportHeaders as n, parseLeadImportCsv as o, buildLeadImportRows as r, LEAD_IMPORT_FIELDS as t };
