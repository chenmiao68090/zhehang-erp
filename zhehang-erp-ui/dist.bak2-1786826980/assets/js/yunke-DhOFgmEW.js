...
//#region src/api/yunke.ts
var yunkeApi = {
	/** 读配置(signKey 脱敏返回) */
	getConfig: () => get("/crm/yunke/config"),
	/** 保存配置(signKey 留空/掩码则不改) */
	saveConfig: (data) => post("/crm/yunke/config", data),
	/** 测试连接(调云客沟通统计验证凭证/签名/IP) */
	test: () => post("/crm/yunke/test"),
	/** 员工微信列表(真实数据,聚合云客多接口) */
	wechatStaffList: (params) => get("/crm/yunke/wechat-staff-list", params),
	/** 微信聊天:不传talker=会话列表;传talker=该会话消息 */
	wechatChat: (params) => get("/crm/yunke/wechat-chat", params),
	/** 云客账号列表(供员工关联参考) */
	...
	/** 保存员工云客关联(批量) */
	saveUserMap: (data) => post("/crm/yunke/user-map", data),
	/** 给员工开通外呼([{userId,name,phone}]) */
	enableDial: (employees) => post("/crm/yunke/enable-dial", employees),
	/** 点击拨打({phone, userId?}) */
	dial: (data) => post("/crm/yunke/dial", data),
	/** 点击挂断({callId, userId?}) */
	hangup: (data) => post("/crm/yunke/hangup", data),
	/** 新增好友统计({beginYmd,endYmd,pageNum,pageSize}) */
	newFriends: (params) => get("/crm/yunke/new-friends", params),
	/** 某员工朋友圈({wechatId,beginYmd?,endYmd?}) */
	moments: (params) => get("/crm/yunke/moments", params),
	/** 微信语音通话列表({beginYmd,endYmd,callType?,isSend?,pageNum,pageSize}) */
	voiceList: (params) => get("/crm/yunke/voice-list", params),
