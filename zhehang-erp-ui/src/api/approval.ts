...
import type { ProcessDef, TaskItem, InstanceItem } from './workflow'
import type { PageQuery } from './types'

export type { ProcessDef, TaskItem, InstanceItem } from './workflow'
...
export const approvalCenterApi = {
  ...
  /** 抄送我的：后端暂无该端点（见 gapsNoBackend），返回空集占位，便于视图统一渲染 */
  async cc (_params: PageQuery): Promise<{ list: TaskItem[]; total: number }> {
    return { list: [], total: 0 }
  },
