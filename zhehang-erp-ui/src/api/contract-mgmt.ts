...
export interface ContractLinkageRecord {
  time: string
  type: 'renew' | 'terminate' | 'stage_alert' | 'sign'
  title: string
  detail?: string
  by?: string
}
...
export interface BizContract {
  ...
  startDate: string
  endDate: string
  signMethod: 'unset' | 'paper' | 'electronic' | 'fadada' | 'esign'
...
const toSignMethod = (m?: string | null): BizContract['signMethod'] =>
  m === 'online' ? 'esign' : m === 'offline' ? 'paper' : 'unset'
const toSignMode = (m?: string): 'online' | 'offline' =>
  m === 'esign' || m === 'fadada' ? 'online' : 'offline'

/* ---------- 续费阶梯：真实后端 /contract-mgmt/renew-stages（原 localStorage 侧挂已转后端） ---------- */
...
// 从后端拉本租户全部阶梯记录，按合同分组；有记录的合同补全为 6 级（已存的覆盖默认 pending）
async function loadStageOverlay(): Promise<Record<string, ContractStageRecord[]>> {
  try {
    const res = await get('/contract-mgmt/renew-stages', undefined, { silentError: true }).catch(() => null)
    const rows = res ? unwrap<any[]>(res) : []
    const grouped: Record<string, ContractStageRecord[]> = {}
    for (const r of (Array.isArray(rows) ? rows : [])) {
      const cid = String(r.contractId)
      if (!grouped[cid]) grouped[cid] = []
      grouped[cid].push({
        stage: Number(r.stage) as ContractStageRecord['stage'],
        status: (r.status || 'pending') as ContractStageRecord['status'],
        handler: r.handler,
        note: r.note,
        handledAt: r.handledAt
      })
    }
    const map: Record<string, ContractStageRecord[]> = {}
...
function adaptContract(
  ...
  // 仅按合同表真实时间字段生成展示记录，不把未查询的外部发送或任务结果拼成事实。
  ...
  if (s.confirmSignTime) {
    records.push({ time: s.confirmSignTime, type: 'sign', title: '客户签署已确认', by: s.signerTheirs || '客户' })
  }
...
export const contractMgmtApi = {
  async list(
    ...
    const overlay = await loadStageOverlay()
  ...
  async detail(id: number): Promise<BizContract | null> {
    ...
    return adaptContract(s, tplIndex, await loadStageOverlay())
  },
  ...
  async updateStage(payload: {
    ...
    await post(`/contract-mgmt/${payload.id}/stage`, {
      stage: payload.stage,
