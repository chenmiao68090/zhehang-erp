...
//    因此前端的 pending_boss / bossApprove / approvalLevel=3 在后端无对应（见 gapsNoBackend）。
...
// 金额阈值 → 审批级别（仅前端用于展示“预期审批级别”；后端实际只有主管+财务两级，level=3 在后端无对应）
...
function viewStatusToBackend(status?: string): number | undefined {
  switch (status) {
    ...
    // pending_boss 后端无对应节点，不下传（结果为空集）
...
export const orderApi = {
  /** mock 时代用于注入种子数据；真实后端不需要，留空兼容视图 onMounted 调用 */
  async ensureSamples(): Promise<BizOrder[]> {
    return []
  },
  ...
  /**
   * 老板终审：后端无该节点。
   * 兜底退化为财务确认（financeConfirm 后已是 completed），保证视图“老板终审通过”按钮不报错。
   * 实际流程在财务确认即闭环（见 gapsNoBackend）。
   */
