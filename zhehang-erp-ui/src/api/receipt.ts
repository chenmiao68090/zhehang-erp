...
//    - BizReceipt.status      1待确认 2已确认 3已退款 4已驳回
...
//    - 退款审批流（申请→主管审批→财务确认）已接后端 /receipt/refund/* 与 biz_refund_request。
//    - 财务联动时间线已接后端 /receipt/timeline 与 biz_finance_timeline。
...
//    - 入账管理已接后端 /receipt/posting 与 biz_receipt.posting_* 字段,刷新后不丢。
...
export interface ReceiptTimelineEvent {
  ...
  receiptId?: number
  orderId?: number
  type: 'receipt_confirmed' | 'receipt_posted' | 'order_completed' | 'contract_generated' | 'refund_applied' | 'refund_approved' | 'refund_finance_confirmed' | 'refund_completed' | 'refund_rejected' | 'overdue_escalation'
  title: string
  detail: string
  time: string
  operator: string
...
const RECEIPT_STATUS_NUM: Record<string, number> = { pending: 1, confirmed: 2, refunded: 3, rejected: 4 }
function receiptStatusStr(n: any): BizReceipt['status'] {
  return ({ 1: 'pending', 2: 'confirmed', 3: 'refunded', 4: 'rejected' } as Record<number, BizReceipt['status']>)[Number(n)] || 'pending'
}
...
function adaptReceipt(r: Raw): BizReceipt {
  ...
  return {
    ...
    postingAccount: r.postingAccount || '',
    postingOperatorId: Number(r.postingOperatorId || 0),
    postingTime: fmtDateTime(r.postingTime),
...
// ---------- 财务联动时间线：已转真后端 /receipt/timeline（appendTimeline 即发即忘写库，getTimeline 读库） ----------
...
function appendTimeline(evt: Omit<ReceiptTimelineEvent, 'id' | 'time'> & { time?: string }) {
  // 即发即忘:写入后端时间线表(失败静默,不阻塞主流程)
  post('/receipt/timeline', {
...
export const receiptApi = {
  ...
  async confirm(payload: { id: number; confirmerId?: number; confirmerName?: string; status?: 'confirmed' | 'rejected'; remark?: string; actualAmount?: number }): Promise<BizReceipt> {
    ...
    if (payload.status === 'rejected') {
      // 真后端:POST /receipt/reject 置 status=4 已驳回,原因记入备注
      await post('/receipt/reject', {
        receiptId: current.id,
        reason: payload.remark || '',
        operatorId: payload.confirmerId
      })
      appendTimeline({
        receiptId: current.id, orderId: current.orderId,
        type: 'refund_rejected',
        title: `收款驳回 · ${current.receiptNo}`,
        detail: payload.remark || '财务驳回',
  ...
  /** POST /receipt/posting：已确认收款标记入账，入账科目/入账人/入账时间落库 */
  async markPosted(payload: { id: number; postingAccount: string }): Promise<BizReceipt> {
    const raw = unwrap<Raw>(await post('/receipt/posting', {
      receiptId: payload.id,
      postingAccount: payload.postingAccount
    }))
  ...
  // ===== 退款业务（后端审批流:申请→主管审批→财务确认） =====

  /** 退款列表 */
  async getRefunds(params: { page?: number; pageSize?: number; status?: string } = {}): Promise<{ list: RefundRequest[]; total: number }> {
    const res = await get('/receipt/refund/list', { page: params.page || 1, pageSize: params.pageSize || 20, status: params.status }, { silentError: true }).catch(() => null)
    const data: any = res ? unwrap(res) : null
    const list = (data?.records || data?.list || []) as RefundRequest[]
  ...
  async createRefund(payload: Partial<RefundRequest> & { receiptId: number; refundAmount: number; reason: string }): Promise<RefundRequest> {
    ...
    const res = await post('/receipt/refund/apply', {
    ...
    const refund = unwrap<RefundRequest>(res)
    appendTimeline({
  ...
  async approveRefund(payload: { id: number; approverName?: string; approverId?: number; remark?: string }): Promise<RefundRequest> {
    const res = await post(`/receipt/refund/${payload.id}/approve`, {
      approverId: payload.approverId || 0,
      approverName: payload.approverName || '当前主管',
      remark: payload.remark
    })
    const r = unwrap<RefundRequest>(res)
    appendTimeline({
  ...
  async confirmRefund(payload: { id: number; financeConfirmerName?: string; financeConfirmerId?: number; refundWay?: RefundWayKey; remark?: string }): Promise<RefundRequest> {
    const res = await post(`/receipt/refund/${payload.id}/confirm`, {
      financeConfirmerId: payload.financeConfirmerId || 0,
      financeConfirmerName: payload.financeConfirmerName || '财务',
      refundWay: payload.refundWay,
      remark: payload.remark
    })
    const r = unwrap<RefundRequest>(res)
    appendTimeline({
  ...
  async rejectRefund(payload: { id: number; rejectReason?: string; operatorName?: string }): Promise<RefundRequest> {
    const res = await post(`/receipt/refund/${payload.id}/reject`, {
  ...
  async getTimeline(params: { receiptId?: number; orderId?: number; limit?: number } = {}): Promise<ReceiptTimelineEvent[]> {
    const res = await get('/receipt/timeline', {
      receiptId: params.receiptId,
      orderId: params.orderId,
      limit: params.limit ?? 50
    }, { silentError: true }).catch(() => null)
    const rows = res ? unwrap<any[]>(res) : []
  ...
  async getSummary(): Promise<ReceiptSummary> {
    ...
    const refundRes = await this.getRefunds({ pageSize: 500 }).catch(() => ({ list: [] as RefundRequest[], total: 0 }))
    return {
      ...
      refundPending: refundRes.list.filter(r => r.status === 'pending' || r.status === 'approved').length
    }
  }
}
