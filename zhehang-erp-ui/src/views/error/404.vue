  ...
  <div class='error-page'>
    ...
    <div class='error-page__frame'>
      ...
      <div class='error-page__eyebrow'>
        ...
        <span class='error-page__eyebrow-text'>ZHEHANG · ERROR REPORT</span>
        <span class='error-page__line'></span>
      </div>

      <h1 class='error-page__code'>
        <span class='error-page__digit' style='--i:0'>4</span>
        <span class='error-page__digit error-page__digit--accent' style='--i:1'>0</span>
        <span class='error-page__digit' style='--i:2'>4</span>
      </h1>

      <p class='error-page__title'>抱歉，您访问的页面不存在</p>
      <p class='error-page__subtitle'>
        Page Not Found · 请检查地址是否正确，或返回首页继续浏览
      </p>
      ...
      <div class='error-page__actions'>
        ...
        <button type='button' class='error-page__btn error-page__btn--ghost' @click='goBack'>
          <span class='error-page__btn-text'>返回上一页</span>
        </button>
      </div>
...
.error-page {
  ...
  background:
    radial-gradient(ellipse at 20% 30%, rgba(212, 175, 55, 0.08), transparent 55%),
...
.error-page__digit--accent {
  background: linear-gradient(180deg, #F4D87A 0%, #D4AF37 50%, #8a6f1f 100%);
  -webkit-background-clip: text;
  background-clip: text;
  -webkit-text-fill-color: transparent;
}
...
.error-page__btn::before {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(180deg, #F4D87A 0%, #D4AF37 100%);
  opacity: 0;
