  ...
  <div class="order-bill">
    ...
    <section class="table-wrap">
      ...
      >
        ...
        <el-table-column label="服务类型" min-width="220">
          <template #default="{ row }">
            <div class="service-summary">
              <el-tag
                v-for="it in row.items.slice(0, 2)"
                :key="it.id"
                size="small"
                effect="plain"
                class="service-tag"
        ...
        <el-table-column label="审批截止" width="170" align="center">
          <template #default="{ row }">
            <div
              v-if="row.status === 'pending_approval' || row.status === 'pending_finance'"
              class="deadline-cell"
    ...
    >
      <div v-if="approveTarget" class="approve-summary">
        ...
        <div class="summary-row">
          ...
          <span class="val">
            <el-tag
              v-for="it in approveTarget.items"
              :key="it.id"
              size="small"
              effect="plain"
              class="summary-tag"
    ...
    >
      ...
      <div v-if="detailTarget.status === 'rejected'" class="order-reject-note">
        <strong>订单被{{ detailTarget.rejectStage === 'finance' ? '财务' : '主管' }}驳回</strong>
        <p>
      ...
      <template #timeline>
        ...
        <div v-if="detailTarget.submitTime && detailTarget.status !== 'draft'" class="bd-timeline-item">
          <i class="bd-timeline-dot" :class="{ success: ['pending_finance', 'completed'].includes(detailTarget.status) }" />
          <div>
...
// ===== 客户下拉(真实接口 /crm/customer/list) =====
const customerOptions = ref<Array<{ id: number; name: string; contact: string; phone: string }>>([])

async function loadCustomerOptions() {
  try {
    // 拉真实客户(已删/逻辑删除由后端 deleted=0 自动过滤);量>200 再考虑翻页
    const res: any = await customerApi.list({ pageNum: 1, pageSize: 200 })
    // request.ts 响应拦截器返回完整 R 信封 {code,data,message},分页数据在 res.data.records(拦截器另补了 list 别名)
    const records: Array<Record<string, any>> =
      res?.data?.records || res?.data?.list || res?.records || (Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []))
...
const approveMode = ref<'manager' | 'finance'>('manager')
const approveTarget = ref<BizOrder | null>(null)
const approveOpinion = ref('')

// ===== 驳回对话 =====
const rejectVisible = ref(false)
const rejectStage = ref<'manager' | 'finance'>('manager')
...
const rejectDialogTitle = computed(() => {
  const stage = rejectStage.value === 'manager' ? '主管' : '财务'
...
async function openDetailById(orderId: number) {
...
onMounted(async () => {
  ...
  if (Number.isSafeInteger(deepLinkOrderId) && deepLinkOrderId > 0) {
    try {
      await openDetailById(deepLinkOrderId)
    } catch {
...
.order-bill {
  --gold: #3370ff;
  --gold-soft: rgba(51, 112, 255, 0.55);
  --gold-line: rgba(51, 112, 255, 0.18);
  --bg-card: #16161E;
  --bg-card-2: #1A1A24;
  --text-primary: #F5F5F5;
  --text-body: #B8B8C0;
...
.page-header {
  ...
  &::before {
    ...
    inset: 0;
    background:
      radial-gradient(circle at 88% 20%, rgba(51, 112, 255, 0.18), transparent 45%),
      radial-gradient(circle at 8% 90%, rgba(51, 112, 255, 0.12), transparent 50%);
    pointer-events: none;
  }
}
.header-meta {
  ...
  .meta-divider { width: 24px; height: 1px; background: rgba(51, 112, 255, 0.4); }
  .meta-time { color: rgba(51, 112, 255, 0.6); }
}
...
.page-title {
  ...
  .title-cn {
    ...
    background: linear-gradient(180deg, #eaf1ff 0%, #3370ff 100%);
...
.header-actions {
  ...
  .primary-btn {
    background: linear-gradient(135deg, #3370ff 0%, #B8941F 100%);
    border: 1px solid rgba(51, 112, 255, 0.6);
    color: #1A1A24;
    font-weight: 600;
    box-shadow: 0 0 18px rgba(51, 112, 255, 0.25);

    &:hover {
      background: linear-gradient(135deg, #E5C24F 0%, #3370ff 100%);
...
.stat-card {
  ...
  &.active {
    ...
    box-shadow: 0 0 18px rgba(51, 112, 255, 0.2);
  }
...
:deep(.status-tabs .el-tabs__nav-wrap::after) { background: rgba(51, 112, 255, 0.1); }
...
.order-table {
  ...
  --el-table-header-bg-color: rgba(51, 112, 255, 0.04);
  --el-table-row-hover-bg-color: rgba(51, 112, 255, 0.06);
  --el-table-border-color: rgba(51, 112, 255, 0.1);
...
:deep(.order-dialog .el-dialog__header) {
  ...
  margin-right: 0;
  background: linear-gradient(90deg, rgba(51, 112, 255, 0.08), transparent);
}
...
:deep(.order-dialog .el-dialog__footer) {
  ...
  border-top: 1px solid rgba(51, 112, 255, 0.12);
}
...
.section-head {
  ...
  .section-bar {
    ...
    background: var(--gold);
    border-radius: 1px;
    box-shadow: 0 0 8px rgba(51, 112, 255, 0.6);
  }
  .section-title {
    font-size: 14px;
    font-weight: 600;
...
.items-table {
  --el-table-bg-color: transparent;
  --el-table-tr-bg-color: transparent;
  --el-table-header-bg-color: rgba(51, 112, 255, 0.06);
...
.items-summary-bar {
  ...
  padding: 12px 16px;
  background:
    linear-gradient(90deg, rgba(51, 112, 255, 0.08), rgba(51, 112, 255, 0.02)),
    repeating-linear-gradient(45deg, transparent 0 6px, rgba(51, 112, 255, 0.03) 6px 7px);
  border: 1px solid rgba(51, 112, 255, 0.22);
  border-radius: 8px;
  font-size: 12px;
  color: var(--text-body);
  ...
  .sum-chip {
    ...
    &.emph {
      ...
      box-shadow: 0 0 12px rgba(51, 112, 255, 0.1);
    }
  }
}
.cell-final {
...
.upload-placeholder {
  ...
  border: 1px dashed rgba(51, 112, 255, 0.32);
  border-radius: 6px;
  color: rgba(51, 112, 255, 0.7);
  font-size: 13px;
  cursor: pointer;
...
:deep(.detail-drawer .el-drawer__header) {
  ...
  background: linear-gradient(90deg, rgba(51, 112, 255, 0.1), transparent);
  color: var(--gold);
  font-size: 16px;
  font-weight: 600;
  letter-spacing: 0.04em;
...
.linkage-card {
  ...
  background:
    linear-gradient(135deg, rgba(102, 177, 255, 0.06), rgba(51, 112, 255, 0.04)) !important;
...
.stat-card {
  ...
  &.theme-primary .stat-num { color: #3370ff; }
