  ...
  <div class="contract-mgmt">
    ...
    <section class="stat-strip">
      <div class="stat-cell s-active">
        ...
        <div class="cell-label">已签署合同</div>
      </div>
      ...
      <div class="panel-head">
        ...
        <div class="ph-right">
          <el-input
            ...
            size="default"
          />
          <el-button @click="loadList()"><el-icon><RefreshRight /></el-icon>刷新</el-button>
        </div>
      </div>
    ...
    >
      <el-form ref="genFormRef" :model="genForm" :rules="genRules" label-width="120px">
        ...
        <el-form-item label="合同名称" prop="title">
          <el-input v-model="genForm.title" maxlength="200" show-word-limit />
        </el-form-item>
    ...
    >
      ...
      <div class="contract-sign-grid">
        ...
        <div class="contract-sign-card">
          <span>送签登记</span>
          <b>{{ detailDrawer.data.partyBSignTime ? signMethodLabel(detailDrawer.data.signMethod) : '未登记' }}</b>
          <small>{{ detailDrawer.data.partyBSignTime || '—' }}</small>
        </div>
        <div class="contract-sign-card">
          <span>附件记录</span>
          <b>{{ detailDrawer.data.contractFileUrl ? '已有附件字段' : '未记录附件' }}</b>
          <small>当前页面未接入鉴权下载</small>
        </div>
      </div>
      ...
      <template #timeline>
        ...
        <div v-for="(rec, i) in linkageRecords" :key="'lk-' + i" class="bd-timeline-item">
          <i class="bd-timeline-dot" :class="{ success: rec.type === 'sign' || rec.type === 'renew' }" />
          <div>
      ...
      <template #footer>
        ...
        <el-button
          v-if="detailDrawer.data.signStatus === 'sent' || detailDrawer.data.signStatus === 'partial_signed'"
          type="success"
    ...
    <el-dialog v-model="stageDialog.visible" title="阶梯处理记录" width="520px" destroy-on-close>
      ...
      <el-form :model="stageDialog.form" label-width="100px" style="margin-top: 12px">
        <el-form-item label="处理人">
          <el-input v-model="stageDialog.form.handler" maxlength="50" placeholder="请输入实际处理人姓名" />
        </el-form-item>
...
const detailHistory = computed(() => {
  ...
  if (d.partyBSignTime) items.push({ time: d.partyBSignTime, action: '送签状态已登记', by: '系统记录', color: '#409eff' })
  if (d.partyASignTime) items.push({ time: d.partyASignTime, action: '客户签署已确认', by: d.partyASigner || '客户', color: '#67c23a' })
...
async function sendSign(row: BizContract) {
  await ElMessageBox.confirm(
    `确认登记合同「${row.contractNo}」已线下送交客户签署？本操作只记录送签状态和时间，不会发送文件或调用电子签平台。`,
    '登记已送签',
    { type: 'warning', confirmButtonText: '确认登记' }
  )
    .then(async () => {
      await contractMgmtApi.sendSign({ id: row.id, signMethod: 'paper' })
...
async function confirmSign(row: BizContract) {
  await ElMessageBox.prompt('请输入甲方签署人姓名以确认签署完成', '确认签署', {
    ...
    await contractMgmtApi.confirmSign({ id: row.id, signer: value })
...
.stat-cell.s-renew  .cell-num   { color: #3370ff; }
