        ...
        <div class="panel posting-panel">
          ...
          <el-table :data="confirmedReceipts" stripe class="zh-table">
            ...
            <el-table-column label="入账状态" width="140" align="center">
              <template #default="{ row }">
                <el-tag v-if="isPosted(row.id)" type="success" effect="dark" size="small">已入账</el-tag>
                <el-tag v-else type="warning" effect="plain" size="small">未入账</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="入账科目" width="180" align="center">
              <template #default="{ row }">
                <span v-if="isPosted(row.id)" class="account-tag">{{ getPostingAccount(row.id) }}</span>
                <span v-else class="muted">—</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="180" fixed="right">
              <template #default="{ row }">
                <el-button v-if="!isPosted(row.id)" type="primary" link size="small" @click="openPostingDialog(row)">
                  标记入账
                </el-button>
...
async function loadRefundableOrders() {
  try {
    ...
    refundableOrders.value = (res.list || []).filter((o: BizOrder) => o.status === 'completed' || o.status === 'pending_finance' || o.status === 'pending_boss')
  } catch {
...
.page-title {
  ...
  .title-cn {
    background: linear-gradient(135deg, #F5F5F5 0%, #D4AF37 60%, #10B981 100%);
...
:deep(.el-divider) { border-color: rgba(212, 175, 55, 0.12); }
...
.ov-purple { border-left: 4px solid #2563eb; }
.ov-red,
.ov-amber,
