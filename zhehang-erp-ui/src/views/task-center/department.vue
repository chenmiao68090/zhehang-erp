  ...
  <div class="task-page task-department">
    ...
    <header class="page-header">
      ...
      <div class="header-main">
        <h1 class="page-title">
          <span class="title-cn">项目部任务</span>
          <span class="title-en">Project Mission</span>
        </h1>
        <p class="page-desc">由项目经理统筹的多人协作型综合项目任务派发与全流程闭环跟踪</p>
      </div>
      <div class="header-actions">
        <el-button type="primary" class="btn-gold" @click="openCreate">+ 新建项目部任务</el-button>
      </div>
    </header>
    ...
    <section class="filter-bar">
      <div class="filter-left">
        ...
        <el-select v-model="filters.subType" placeholder="项目类型" clearable style="width: 160px" @change="loadList">
    ...
    <section class="content-section">
      <div class="section-head">
        <h2 class="section-title">项目任务列表</h2>
        <span class="section-sub">PROJECTS / TASKS · {{ list.length }} 条</span>
      </div>
    ...
    <!-- 创建任务 Dialog -->
    <el-dialog v-model="dlg.create" title="新建项目部任务" width="640px">
      <el-form :model="form" label-width="100px" label-position="right">
        ...
        <el-form-item label="任务名称">
          <el-input v-model="form.taskName" placeholder="如：杭州数智-财税合规年度项目" />
        </el-form-item>
        ...
        <el-form-item label="客户交付物">
          <el-input v-model="form.deliverable" placeholder="如：项目结项报告、审计底稿" />
        </el-form-item>
        <div class="form-row">
    ...
    <BusinessDetailDrawer
      ...
      :subtitle="`${current.customerName || '—'} · ${current._subType || '项目部任务'}`"
      eyebrow="项目部业务"
      :avatar="current._subType?.slice(0, 2) || '项'"
    ...
    >
      ...
      <template #meta>
        <div class="bd-kv-grid">
          ...
          <div class="bd-kv"><span>项目类型</span><b>{{ current._subType || '—' }}</b></div>
...
async function doCreate() {
  if (!form.taskName || !form.subType) return ElMessage.warning('请填写任务名称与项目类型')
  await taskApi.create({
    ...
    remark: buildRemark('C', form.subType, '')
  })
  ElMessage.success('已创建项目部任务')
  dlg.create = false
  loadList()
}
