...
// 任务管理 = 原[一次性任务 + 周期性任务 + 老板安排任务]三个独立菜单合并为一页三标签。
// 三者本是同一张 biz_task 表按 remark 前缀 A/B/D 切分,合并消除重叠菜单;复用现有三个组件,逻辑零改动。
// lazy 让非当前标签不预加载(避免三个 taskApi 一次性并发)。支持 ?tab= 直接定位(旧 once/periodic/boss 重定向带上)。
import TaskOnce from './once.vue'
import TaskPeriodic from './periodic.vue'
import TaskBoss from './boss.vue'

const route = useRoute()
const activeTab = ref(['once', 'periodic', 'boss'].includes(route.query.tab as string) ? (route.query.tab as string) : 'once')
</script>
